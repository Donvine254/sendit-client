...
export async function createParcel(parcelData, router, setOrderData, setCreatedParcel) {
  try {
...
export async function updateUserDetails(currentUser, phone_number, setCurrentUser){
  //pass the phone_number and current user
  try {
    if(phone_number && !currentUser.phone_number){
      const response = await Axios.patch(`http://127.0.0.1:4000/users/${currentUser.user_id}`,{phone_number: phone_number});
      const data = await response.data
      console.log(data)
      setCurrentUser((prev)=>({
        ...prev,
